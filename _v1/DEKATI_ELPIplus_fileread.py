...
def file_reader(filename):
    ...
    if filename[-3:] == "dat":  # for dat file directly aqcuired from elpi
        header_lines = 40
        timeconverter = lambda x: datetime.strptime(
            x.decode("utf-8"), "%Y/%m/%d %H:%M:%S.%f"
        )
    ...
    current = np.genfromtxt(
        filename, delimiter=",", skip_header=header_lines, usecols=(range(2, 16, 1))
    )
    concentration = np.genfromtxt(
        filename, delimiter=",", skip_header=header_lines, usecols=(range(34, 48, 1))
    )
...
def particles_per_stage(concentration, time):
    ...
    for stage in range(
        14
    ):  # iterate through all stages, to obtain the particle number for each stage separately
        ...
        for k in range(0, len(time) - 1):  # iterate through all measurement_points
            t_interval = (
                time[k + 1] - time[k]
            ).total_seconds()  # calculate the time between two measurement points
            particles_interval = (
                concentration[k, stage] * flow * t_interval
...
def mass_per_stage(n_particles_stage):
    ...
    for stage in range(14):
        ...
        m = (
            n_particles_stage[stage] * mi * (10**6)
        )  # calculation of total mass per stage
        m_stage.append(m)
