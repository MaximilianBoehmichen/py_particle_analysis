...
The electrometer has a noise current of 1x10^15 ampere according to the manual
...
from _v1.defs import device_list, elementary_charge
...
def import_data(filename, data_choice=""):
    ...
    flowrate = input(
        "Please enter the used flow rate as float in L/min"
    )  # could be logged
    ...
    )  # arbitrary start time as it is not
    ...
    # flow_list just runs with it filling the same value constantly, as the flow is not logged currently

    for k in range(1, len(data)):  # runs through all the data line by line
        if (
            data[k, 0] == 0
        ):  # when the first column value in the line is 0, no sample was saved, so it is skipped
            continue
        elif (
            data[k, 0] != 0 and data[k, 0] != data[k - 1, 0]
        ):  # when the first column value is not zero and is
            # different from the last first column value, a new measurement is started
            if (
                len(sub_el_time) == 0
            ):  # if the length of the sub... list is 0, no measurement was saved in it before
                sub_em_voltage.append(
                    data[k, 2]
                )  # so only the new measurement is appended to it
                ...
                em_voltage.append(
                    sub_em_voltage
                )  # transferred to em_voltage and tot_el_time
                tot_el_time.append(sub_el_time)
                flow_list.append(sub_flow_list)
                scan_nr.append(
                    counter
                )  # also a scan number is added to the scan_nr list
                ...
                sub_em_voltage.append(
                    data[k, 2]
                )  # the first entry of the new measurement is saved to the sub... lists
                sub_el_time.append(data[k, 1])
                sub_flow_list.append(flowrate)
        elif (
            data[k, 0] != 0 and data[k, 0] == data[k - 1, 0]
        ):  # if the first column value is not zero and is the same as
            sub_em_voltage.append(
                data[k, 2]
            )  # the value before, the measurement entry  belongs to the same
            sub_el_time.append(
                data[k, 1]
            )  # measurement, so it is added to the sub... list
            sub_flow_list.append(flowrate)
    em_voltage.append(
        sub_em_voltage
    )  # at the end of the loop, the sub lists have to be saved into em_voltage and
    tot_el_time.append(
        sub_el_time
    )  # tot_el_time to also save the last measurement taken
    flow_list.append(sub_flow_list)
    ...
    max_length = max(
        map(len, tot_el_time)
    )  # get length of longest measurement for preallocation of arrays

    # Preallocation of all arrays and lists that should be exported
    el_time = np.full((nr_scans, max_length), np.nan)
    Cn = np.full((nr_scans, max_length), np.nan)
    flow_lpm = np.full((nr_scans, max_length), np.nan)
    start_time = []

    for k in range(nr_scans):
        for i in range(
            len(tot_el_time[k])
        ):  # fill el_time and Cn arrays with values from lists of lists
            el_time[k, i] = tot_el_time[k][i] - tot_el_time[k][0]
            Cn[k, i] = voltage_to_conc(
                em_voltage[k][i], flowrate_ccs
            )  # calculate concentration from voltage values
            flow_lpm[k, i] = flow_list[k][i]
        start_time.append(
            start + timedelta(0, tot_el_time[k][0])
        )  # add elapsed time at beginning of the
    ...
    # passed flow_lpm in add_info as list, when more actions should be done with flow rate, maybe move to own dict entry
...
if __name__ == "__main__":
    ...
    data_dict = import_data_dict(
        device_list.query("Import_Script=='TSI_EM3068_fileread'")[
            "Device_Identifier"
        ].values[0],
        filename,
    )
